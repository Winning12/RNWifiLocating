...
export default class ListItemElement extends Component {
    constructor(props){
        ...
        this.state={
            fold:true,
            data:[],
            classroom:{"course":"","roomName":""},
        }
    }

    componentDidMount(){
        var formData=new FormData();
        formData.append("roomName",this.props.name)
        fetch('http://118.25.56.186:8080/api/classroom/getByName', {
        method: 'POST',
        headers: {
        ...
        .then((response) => {
            ...
            if(json.status=="FAIL")
                ...
                this.setState({data:json.detail,classroom:{"course":"","roomName":this.props.name}})
            }
            this.getCourse(this.props.name)//进一步查询获得该教室当前课程信息
        })
        .catch((error) => {
            alert(error)
            if (error) {
            ...
            this.setState({classroom:{"course":"","roomName":this.props.name}})
        });
    }
    ...
    renderTitle=()=>{
        return(
            ...
            onPress={()=>{
                ...
                this.props.setDes(this.props.name)
            }}
